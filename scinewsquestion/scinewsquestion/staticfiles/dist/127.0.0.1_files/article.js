...
function insertQuestions(){
    ...
    for (var i=0; i<questions.length; i++){ 
        ...
        newbutton.setAttribute("class","btn squarebox");
        newbutton.innerHTML='<i class="fa fa-square-o"></i>';
        newQ.appendChild(newbutton);
...
function reconSidebar(){
 //Floating sidebar
   ...
   // Initialize popover -> delete later
   var refbtnno=0;
   var reftotal=document.getElementsByClassName("refbtn").length;
   var refs=[];
   for (var i=0;i<reftotal;i++){
       var eachref=[];
       refs.push(eachref);
   }
   $('.refbtn').each(function(){
       this.setAttribute("id", "refbtn"+refbtnno.toString());
       refbtnno=refbtnno+1;
       $(this).popover({
           title: 'Add Your Reference',
           html: true,
           content:'Select text range you referred to answer this question. <div class="row"> <div class="col-xs-8" style="padding-right:5px"><input id="ref_input" type="text" placeholder="Select text range." class="form-control"></div>'+
           '<div class="col-xs-4" style="padding-left:5px;"><button class="btn btn-add" id="addRefbtn" type="button">Add</button></div></div>',
           placement:'bottom',
       })
       .on('shown.bs.popover', function(){
           $('#ref_input').focus();
           var refno=this.id.substring(6);
           if(refs[refno].length>0){
               $('#ref_input')[0].value=refs[refno][0];
               var addrefbtn=$('#addRefbtn')
               addrefbtn[0].textContent="Change"
           }
           else{
               var addrefbtn=$('#addRefbtn')
               addrefbtn[0].textContent="Add"
           }
       })
       .on('click', function(e) {
           $('.refbtn').not(this).popover('hide');
       })
       $(this).tooltip({
           placement: 'bottom',
           title: 'Click to add your reference.',
           animation: true,
           delay: {show:200, hide:10}
       });
   });  
